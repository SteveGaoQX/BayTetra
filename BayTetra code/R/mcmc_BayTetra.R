...
#' \deqn{y_{i q j}=\boldsymbol{Z}_{i j}^{\mathrm{T}} \boldsymbol{\alpha}_q+\sum_{l=1}^{L-1} \widetilde{\beta}_{K q l} \widetilde{\boldsymbol{B}}_l\left(t_{i q j}\right)+\sum_{k=1}^{K-1} \mathbb{I}\left(g_i=k\right)\left(\widetilde{\beta}_{k q 0}+\sum_{l=1}^{L-1} \widetilde{\beta}_{k q l} \widetilde{\boldsymbol{B}}_l\left(t_{i q j}\right)\right)+\omega_{i q}+\epsilon_{i q j},}
#'
#' \deqn{ \boldsymbol{\omega}_i=\left(\omega_{i 1}, \ldots, \omega_{i Q}\right) \sim \mathcal{N}\left(\mathbf{0}, \Sigma_\omega\right), \epsilon_{i q j} \sim \mathcal{N}\left(0, \sigma_q^2\right),}
#'
#' where \eqn{\widetilde{\boldsymbol{B}}_l\left(t_{i q j}\right)} denote the \eqn{l}-th basis function for the \eqn{L-1} dimensional
#' cubic B-spline expansion at time \eqn{t_{iqj}.}
#'
