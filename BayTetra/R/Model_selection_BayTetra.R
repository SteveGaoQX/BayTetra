...
Model_selection_BayTetra = function(df_min,df_max,
  ...
  for (df in df_min:df_max) {
    ...
    post_tau_samples = mcmc_result$pos.tau_q
    post_lambda = mcmc_result$pos.lambda_q
    ...
    model_llh_matrix = array(0, dim = c(I, post_sample_length))
    for (c in 1:post_sample_length) {
      ...
      current_tau = post_tau_samples[c,];
      current_lambda = post_lambda[c,];
      current_sigma2 = post_sigma_samples[c, ];
      # Initialize a vector to store the log likelihood values
      sum_ll = vector("numeric", length = I)
      # Assuming that total_records equals I and other necessary variables are defined earlier
      for (i in 1:I) {
        k = g[i]
        J_iq = length(which(data_index[i,1,] == 1)) # Assuming J_iq is constant for all q
        # Construct the observed value y_iqj, which is a vector of length Q*J_iq
        y_iqj = vector("numeric", length = Q * J_iq)
        mean = vector("numeric", length = Q * J_iq)
        # Start filling in the y_iqj vector and constructing the mean
        for (q in 1:Q) {
          data_index_iq = which(data_index[i,q,] == 1)
          if (k == K) {
            mean[((q-1)*J_iq + 1):(q*J_iq)] = Z[i,q,data_index_iq, ]%*%current_alpha[q,] +
              B[i,q,data_index_iq, ]%*%current_beta[K,q,]
          } else {
            mean[((q-1)*J_iq + 1):(q*J_iq)] = Z[i,q,data_index_iq, ]%*%current_alpha[q,] +
              B[i,q,data_index_iq, ]%*%current_beta[K,q,] +
              B[i,q,data_index_iq, ]%*%current_beta[k,q,]
          }
          # Fill y_iqj with observed data
          y_iqj[((q-1)*J_iq + 1):(q*J_iq)] = y[i,q,data_index_iq]
        }
        # Construct the cov_all matrix
        theta_all = matrix(0, nrow = Q*J_iq, ncol = Q*J_iq)
        omega_all = matrix(0, nrow = Q*J_iq, ncol = Q*J_iq)
        sigma_all = matrix(0, nrow = Q*J_iq, ncol = Q*J_iq)
        for (q in 1:Q) {
          # Construct cov_mat for each q
          tau_q = current_tau[q]
          lambda_q = current_lambda[q]
          cov_mat = diag(J_iq)
          for (j in 1:J_iq) {
            for (j_prime in (j + 1):J_iq) {
              if (j_prime <= J_iq){
                diff = (t[i,q , data_index_iq[j]] - t[i,q , data_index_iq[j_prime]])/lambda_q
                cov_value = exp(-diff^2)
          ...
          theta_all[((q-1)*J_iq + 1):(q*J_iq), ((q-1)*J_iq + 1):(q*J_iq)] = cov_mat
          # Fill in sigma_all for each q
          sigma_block = diag(rep(current_sigma2[q], J_iq))
          sigma_all[((q-1)*J_iq + 1):(q*J_iq), ((q-1)*J_iq + 1):(q*J_iq)] = sigma_block
        }
        for (q1 in 1:Q) {
          for (q2 in 1:Q) {
            # Fill in omega_all for each q
            omega_block = matrix(current_Sigma_omega[q1, q2], J_iq, J_iq)
            omega_all[((q1-1)*J_iq + 1):(q1*J_iq), ((q2-1)*J_iq + 1):(q2*J_iq)] = omega_block
          }
        }
        # Sum up theta_all, omega_all, sigma_all to get cov_all
        cov_all = theta_all + omega_all + sigma_all
        # Compute the log likelihood for individual i
        sum_ll[i] = dmvn_rcpp(y_iqj, mean, cov_all, logd = TRUE)
      }
